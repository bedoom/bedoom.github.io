...
* 进程控制
  ...
  3. 进程的终止
     ...
     ```flow
     st=>start: 开始
     op1=>operation: 检索该进程的PCB
     op2=>operation: 终止该进程的执行
     op3=>operation: 设置调度标志为真
     op4=>operation: 终止子孙进程
     op5=>operation: 归换全部资源
     op6=>operation: 将被终止的PCB在所在队列中移出
     cond1=>condition: 是否处于执行状态？
     cond2=>condition: 有无子孙进程？
     e=>end: 结束
     
     st->op1->cond1
     cond1(no)->cond2
     cond1(yes, right)->op2->op3(left)->cond2
     cond2(no)->op5
     cond2(yes, right)->op4->op5
     op5->op6->e
